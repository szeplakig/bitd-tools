...
import {AiOutlineUnlock as UnlockedIcon, AiFillLock as LockedIcon} from 'react-icons/ai';
...
const rollClientTarget = (type) => {
    ...
    if (el.includes("(roll again)")) {
        const allClientTarget = score_generator_data.client_target.map(e => {
            return e.list;
        }).flat(1).filter(e => {
...
const ScoreGenerator = () => {
    ...
    const handleRollClick = () => {
        if (!clientTargetTypeLock) {
            setClientTargetType(rollArray(score_generator_data.client_target.map(e => e.type)));
        ...
        >
            ...
            >
                <Card 
                    header={
                        <div className='flex items-center justify-center'>
                            <label htmlFor='clientTargetType' className='font-bold'>Client / Target</label>
                            <select
                                id='clientTargetType'
                            ...
                            <LockButton locked={clientTargetTypeLock} setLocked={setClientTargetTypeLock}/>
                            <DiceButton onClick={() => {
                                if (clientTargetTypeLock) {
                                    return;
                                }
                    ...
                    content={
                        <div>
                            <div
                                className='mb-2 bg-bitdDarkGray flex items-center justify-center'
                            >
                                <LockButton locked={clientTargetLock} setLocked={setClientTargetLock}/>
                                <DiceButton onClick={() => {
                                    if (clientTargetLock) {
                                        return;
                                    }
                ...
                <Card 
                    header={
                        <div className='flex items-center justify-center'>
                            <label htmlFor='workType' className='font-bold'>Work</label>
                            <select
                                id='workType'
                                className='cursor-pointer ml-3 bg-bitdBlack'
                                value={workType}
                ...
                <Card 
                    header={
                        <div className='flex items-center justify-center'>
                            ...
                            <LockButton locked={connectionLock} setLocked={setConnectionLock}/>
                            <DiceButton onClick={() => {
                                if (connectionLock) {
                                    return;
                                }
...
const Card = ({header, content}) => {
    return (
        ...
        >
            <div
                className='px-2 py-1 bg-bitdBlack w-full'
            >
                {header}
            </div>
            <div
